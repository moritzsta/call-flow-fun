...
type FelixFormData = z.infer<typeof felixSchema>;

interface SingleFinderFelixDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onStart: (config: { city: string; state: string; category: string; maxCompanies?: number }) => void;
  isStarting: boolean;
}
...
export const SingleFinderFelixDialog = ({
  ...
  const {
    ...
    watch,
    reset,
  } = useForm<FelixFormData>({
    resolver: zodResolver(felixSchema),
    defaultValues: {
      city: '',
      state: '',
      category: '',
  ...
  const onSubmit = (data: FelixFormData) => {
    onStart({
      city: data.city,
      state: data.state,
  ...
  return (
    <AdaptiveDialog
      ...
      description="Starten Sie die Firmensuche mit Felix"
    >
